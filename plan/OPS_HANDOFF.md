...
- [x] Zero Trust enabled by human; app `speedbd-admin` created via API 2026-09-16 (team renamed `falling-scene-e081` → `scbd.cloudflareaccess.com` by human; verifier + secrets updated to match). Destinations MUST cover pages AND every browser-called admin API: `/admin`, `/admin/*`, `/api/admin`, `/api/admin/*`, `/api/uploads`, `/api/uploads/*` (API pairs added after real 401s — page loads prove login, but `fetch()` calls bypass Access without them; uploads fixed 2026-09-16 the same way).
