...
| A6 | [x] | Registration guide + manual payment v1 — WCA account → register on our site (WCA-validated) → bKash/Nagad Send-Money → TxID form → human verification in payment dashboard | Unblocks comps without merchant account | Guide: D1; Tx: D1 queue | Pages + Worker + D1 + Turnstile | M | A5, A17, A19 |
...
| A16 | [x] | Records `/records` (promoted from B1) — BD NR table (like SCA), history, hero cards, "updated daily, source: WCA export" | Prestige + media proof | Unofficial `ranks`/`results` cache | Cron → KV → Pages | M | ARCH cache |
| A17 | [x] | WCA auth + user dashboard `/dashboard` — log in with WCA OAuth, validate WCA ID, register on our site, see my registrations, payment status, competition history | One login, verified identity, no duplicate profiles; long-term path to own registration UX | WCA OAuth + D1 `competitor`/`registration` | Pages + Worker + D1 | L | A4, A6 |
| A18 | [x] | Admin dashboard `/admin` (Access-gated) — manage all content: announcements, pages, people, sponsors, FAQ, news, comp overrides, Worlds page, donation totals. TipTap rich-text, fixed toolbar, JSON → sanitized HTML | Repo holds no content; non-devs publish without deploys | D1 | Pages + Worker + D1 + Access | L | — |
...
| A20 | [x] | News `/news` + `/news/[slug]` — comp recaps, NR posts, Worlds journey, managed in admin dashboard | Media coverage home (no forum in v1) | D1 via admin (TipTap) | Pages ISR | S | A18 |
| A21 | [x] | Competition history — general archive (past comps + podiums link-out) + per-user history in `/dashboard`. Replaces regional sub-pages | "What have we run, what did I attend?" without region fragmentation | WCA cache + D1 registrations | Pages ISR | M | A4, A17 |
...
- Public volunteer tiers, public DOB/phones/addresses, YouTube URLs in image fields (stills only).
