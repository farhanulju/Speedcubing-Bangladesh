...
Updated: 2026-09-17 (LIVE: Pages deployed, sync running in prod, both logins tested; production-readiness patch is local and not deployed). Newest first. This is the file the next agent reads to avoid reconstructing context from chat.
...
- Production UI review continued 2026-09-17 in the signed-in Chrome profile. Live desktop routes had no console errors or document overflow, but the public mobile menu opened below the viewport; `/people` attempted to render an invalid external URL as an R2 key; the dashboard lacked an `h1`; production lacked canonical/social/sitemap/robots metadata and baseline security headers; and a published test person remains visible. Findings: `plan/PRODUCTION_UI_QA_2026-09-16.md`.
- A second local production-readiness patch is implemented but not deployed: mobile menu positioning/current-page state, safe media-key rendering, retired WCA event names, dashboard heading and collapsed under-18 consent guidance, route-specific SEO/canonical/social metadata, generated social card, robots/sitemap, security/privacy/cache headers, safer 404/500 session behavior, honest privacy/Worlds/contact copy, valid home card markup, and required admin-choice placeholders. The clean seeded Playwright e2e and expanded 11-check smoke suite are green at 360px; local headers and the social image were visually checked in Chrome. Real content, owner-approved privacy/retention language, per-user historical results, analytics, custom domain, and production deployment/retest remain.
...
- AGENTS next: review/commit the local production-readiness patch, then deploy and retest production only after approval. A21 per-user historical WCA result data needs a cache/source design; content and privacy retention/rights need owner review. Stitch-only gaps (tasks view, hero stats/photos, Hall of Fame) and A15 analytics/link-in-bio remain subject to approved scope; WP-53 cutover checklist.
