...
- Production UI review completed 2026-09-16 in the signed-in Chrome profile; after the user's CMS authorization fix, an authorized test page draft was created and reopened successfully (slug `codex-qa-smoke-2026-09-16`, still unpublished). Follow-up UX issues were validated read-only: homepage Turnstile runs before intent, WCA event codes are unlabeled, discovery filters are sparse, admin forms expose raw schema fields, and the donation-page create path is missing. Findings and screenshot index: `plan/PRODUCTION_UI_QA_2026-09-16.md`; CMS save issue resolved on retest.
