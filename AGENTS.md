...
| Path | Role |
|------|------|
| `docs/` | Durable meaning: vision, features, architecture, memory. |
| `plan/` | Execution: roadmap, MVP scope, review checklists. |
| `web/` | Website code (Cloudflare Pages + Workers + D1 + R2). English v1. |
| `docs/project-memory/` | `CURRENT_STATE.md`, `DECISION_REGISTER.md`, `DEVELOPMENT_LOG.md` |

Do not create top-level folders without a decision entry. Do not store secrets (`.env`, API tokens, bKash numbers for production) in git. Use Cloudflare dashboard secrets + `.dev.vars` locally (gitignored).

## 2. Hard rules

- **English only for v1.** No `bn/` routes, no Bangla CMS fields, no Bangla fonts. Structure content so a `bn` locale can be added later (all strings in content files, no hardcoded copy in components), but do not ship it.
- **Cloudflare free tier only.** Pages, Workers, D1, KV, R2, Turnstile. No Vercel, Supabase, Firebase, or paid CMS. If free-tier limits block something, log it in `DECISION_REGISTER.md` and degrade gracefully — do not silently add a vendor.
- **WCA is source of truth.** Competitions, results, persons, ranks, records come from WCA official API v0 (OAuth/WCIF writes) and the WST-endorsed unofficial REST API (daily static JSON) via a scheduled Worker cache. Never scrape WCA HTML. Never invent results. Always link back to `worldcubeassociation.org` and `live.worldcubeassociation.org`.
- **One definition per concept.** Volunteer roster shape, competition announcement shape, and donation total logic are defined once (see `docs/architecture/ARCHITECTURE.md`) and reused by `web/`.
- **Worlds fundraising is a separate page** (`/worlds-2027`), not a homepage section. Donation totals are manually updated in v1; automated reconciliation is Phase 2.
- **Volunteer data is public by design** (name, focus area, member-since) but tiers/notes are internal-only. Never render internal fields. Many members are minors — no public DOB, phone, or address. Photo only with consent.
